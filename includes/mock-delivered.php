...
$stmt = $conn->prepare("UPDATE orders SET status = 'delivered' WHERE order_number = ? AND status IN ('pending', 'confirmed', 'shipped')");
...
if ($affected <= 0) {
    ...
    echo json_encode(['ok' => false, 'error' => 'Order not found or already delivered/cancelled']);
    exit;
}
